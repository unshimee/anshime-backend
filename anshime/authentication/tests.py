...
class TestKakaoSignup(TestCase):
    ...
    def test_kakao_signin_should_respond_created_if_user_login_with_kakao_first_time(self):
        ...
        self.assertFalse(content['errors'])

        # Then: 응답 바디에 액세스 토큰, 리프레시 토큰이 있어야 한다.
        decoded_access_token = jwt.decode(
            content['access_token'].encode(), settings.RSA_PUBLIC_KEY_FOR_JWT,
            algorithms=['RS256'],
        )
        self.assertEqual(decoded_access_token['uid'], valid_request_body['kakao_id'])
        self.assertTrue(decoded_access_token['exp'])

        decoded_refresh_token = jwt.decode(
            content['refresh_token'].encode(), settings.RSA_PUBLIC_KEY_FOR_JWT,
            algorithms=['RS256'],
        )
        self.assertTrue(decoded_refresh_token['exp'])
    ...
    def test_kakao_signin_should_respond_ok_if_user_login_with_kakao_again(self):
        ...
        self.assertEqual(decoded_access_token['uid'], second_valid_request_body['kakao_id'])
        self.assertTrue(decoded_access_token['exp'])
